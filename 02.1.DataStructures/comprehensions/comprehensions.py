...
def get_unique_page_ids(records: list[tp.Mapping[str, tp.Any]]) -> set[int]:
    ...
    """
    set_lst = {key['PageID'] for key in records}
    return set_lst
...
def get_unique_page_ids_visited_after_ts(records: list[tp.Mapping[str, tp.Any]], ts: int) -> set[int]:
    ...
    """
    set_lst = {key['PageID'] for key in records if key['EventTime'] > ts}
    return set_lst
...
def get_unique_user_ids_visited_page_after_ts(
    ...
    """
    set_lst = {key['UserID'] for key in records if key['EventTime'] > ts}
    return set_lst
...
def get_events_by_device_type(
    ...
    """
    device_list = [key for key in records if key['DeviceType'] == device_type]
    return device_list
...
def get_region_ids_with_none_replaces_by_default(
    ...
    :return: region ids
    """
    regions_list = [DEFAULT_REGION_ID if key['RegionID'] == None else key['RegionID'] for key in records]
    return regions_list
...
def get_region_id_if_not_none(
    ...
    :return: region ids
    """
    regions_list = [key['RegionID'] for key in records if key['RegionID'] != None]
    return regions_list
...
def get_keys_where_value_is_not_none(r: tp.Mapping[str, tp.Any]) -> list[str]:
    ...
    """
    regions_list = [key for key, value in r.items() if value != None]
    return regions_list
...
def get_record_with_key_in_keys(
    ...
    """
    record_dict = {key: r[key] for key in keys}
    return record_dict
...
def get_keys_if_key_in_keys(
    ...
    """
    record_dict = {key for key in r if key in keys}
    return record_dict